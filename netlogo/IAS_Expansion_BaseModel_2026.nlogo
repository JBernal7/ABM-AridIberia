...
globals [
  ...
  ;; métricas 10 m
  obs_cells_by_year_10m     ;; observado
  sim_cells_by_year_10m     ;; simulación
  survey-mask               ;; for calibration — dataset GIS de la máscara 10 m
...
patches-own [
  ...
  survey10            ;; for calibration (valor numérico aplicado desde el raster (0/1))
...
to setup
  ...
  init-survey-mask               ;; for calibration (máscara 10 m)
  init-survey-occupancy-flags    ;; for calibration (flags)
  set obs_cells_by_year_10m [1365 1365 1365 1365 1403 1403 1403 1403 1403 1403 1459 1459 1459 1459 1486 1486]
;  set sim_cells_by_year_10m []
...
;  plot-size-distribution         ;; histogram of the distribution of the IAS'size (based on surface)
  ...
  ;; --- coherencia de ocupación y acumulado inicial (2008) ---
  refresh-occupancy
  seed-ever-occupied-from-current
  set sim_cells_by_year_10m []
  set sim_cells_by_year_10m lput occupied-cells-inmask-cumulative sim_cells_by_year_10m
...
to set-scenario-params-3B-best
  ;; Best 3B
  set dispersal_radius_m       350
  set kernel_decay             0.0145343
  set maturity-age-years       2
  set min-sup-for-reproduction 71.6672
  set a                        0.0176089
  set b_minus_a                0.464286
  set r_weight_estab           5.00951
  set r_weight_growth          4.62689
  set management_reset_size    11.4280
  set gamma-recovery           2.41726
...
; ----------------------------------------------------------------
; ------------ Calibration only proceedings ------------------------

to init-survey-mask
  set survey-mask gis:load-dataset "abm_prep/survey_mask_10m.asc"
  ;; Aplica el raster a un patch-var numérico
  gis:apply-raster survey-mask survey10
  ;; Binariza el flag de pertenencia a la franja de 10 m
  ask patches [
    set in-survey? (is-number? survey10 and survey10 >= 0.5)
  ]
end

to init-survey-occupancy-flags
  ask patches [
    set ever-occupied? false
    ;; 'occupied?' ya lo reseteas en tu refresh; si quieres arrancar claro:
    set occupied? false
  ]
end

to seed-ever-occupied-from-current
  ask patches with [ in-survey? and occupied? ] [
    set ever-occupied? true
  ]
end

to refresh-occupancy
  ;; 1) limpia ocupación en todos los patches
  ask patches [ set occupied? false ]
  ;; 2) marca ocupados solo los patches que tienen al menos un turtle
  ask patches with [ any? turtles-here ] [
    set occupied? true
  ]
end

to-report occupied-cells-inmask-cumulative
  report count patches with [ in-survey? and ever-occupied? ]
end


to-report objective_inmask_delta
  let obs obs_cells_by_year_10m
  let sim sim_cells_by_year_10m
  let n length obs
  if (n = 0) or (length sim = 0) [ report 1e9 ]

  if length sim < n [
    let lastsim last sim
    set sim (sentence sim n-values (n - length sim) [ lastsim ])
  ]
  if length sim > n [ set sim sublist sim 0 n ]

  let o0 first obs
  let s0 first sim
  let obsΔ map [x -> x - o0] obs
  let simΔ map [x -> x - s0] sim

  let mse 0
  let wsum 0
  let denomN max list 1 (max obsΔ - min obsΔ)

  (foreach (n-values n [i -> i]) obsΔ simΔ
    [[i o s] ->
      let w (1 + (n - 1 - i) / denomN)
      set mse mse + w * (s - o) ^ 2
      set wsum wsum + w
    ])

  let rmse sqrt (mse / wsum)
  let denom max list 1 (last obsΔ)
  report rmse / denom
end
...
to disperse-offspring
  ...
  ;; kernel_a used as dispersal INTENSITY (expected attempts),
...
;    set sup-actual 5
  ...
  refresh-occupancy
  ;; Propaga el acumulado SOLO dentro de la franja 10 m
  ask patches with [ in-survey? and occupied? ] [
    set ever-occupied? true
  ]
  ;; Guarda el valor acumulado en 10 m para la serie simulada (calibración)
;  set sim_cells_by_year_10m lput (count patches with [ in-survey? and ever-occupied? ]) sim_cells_by_year_10m
  set sim_cells_by_year_10m lput occupied-cells-inmask-cumulative sim_cells_by_year_10m
