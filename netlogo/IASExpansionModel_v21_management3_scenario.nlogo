...
;  management-type            ;; "none", "cut", "cut+revegetation", or "erradication" -> Se lleva a interfaz
...
;  import-drawing "abm_prep/logov4.png"
  ...
  set resistancebio-map gis:load-dataset "abm_prep/resistance_0.75_1_def.asc"
...
;     differentiating between "cut", "cut+revegetation" and "erradication".
...
;   2. Reset sup-actual (surface) to small baseline (5 m²), or to 0 m² in erradication.
;   3. Update management memory:
;        - last-managed-tick = current ticks
;        - last-management-type = "cut" or "cut+revegetation"
;        - management-count += 1
;   4. If "cut+revegetation" or "erradication":
...
;    set sup-actual 5
  ...
  ;; Update
  if ticks = 27 [
    export-asc] ;; Export at year 2045
